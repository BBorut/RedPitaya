...
void usage() {
    ...
    const char *format =
            ...
            "  --trig_pa=th    -a th Trig. acq. on ch. 1 (a) pos edge (th=threshold in ADC counts) \n"	    
            "  --trig_pb=th    -b th Trig. acq. on ch. 2 (b) pos edge (th=threshold in ADC counts) \n"	    
            "  --trig_na=th    -x th Trig. acq. on ch. 1 (a) neg edge (th=threshold in ADC counts) \n"	    
            "  --trig_nb=th    -y th Trig. acq. on ch. 2 (b) neg edge (th=threshold in ADC counts) \n"	   
...
int main(int argc, char *argv[])
{
    ...
    static struct option long_options[] = {
            ...
            {"trig_pa",      required_argument, 0, 'a'},
            {"trig_pb",      required_argument, 0, 'b'},
            {"trig_na",      required_argument, 0, 'x'},
            {"trig_nb",      required_argument, 0, 'y'},    
            {"version",      no_argument,       0, 'v'},
            {"help",         no_argument,       0, 'h'},
            {0, 0, 0, 0}
    };
    const char *optstring = "es1:2:a:b:x:y:vh";
    ...
    while ( (ch = getopt_long( argc, argv, optstring, long_options, &option_index )) != -1 ) {
        ...
        case 'a':
	    trig_thr = atof(optarg)/8192*14;
	    t_params[TRIG_SRC_PARAM] = 0;  
            t_params[TRIG_MODE_PARAM] = 1;
            t_params[TRIG_EDGE_PARAM] = 0;
	    t_params[TRIG_LEVEL_PARAM]= trig_thr;	    
	    
            break;
	
        case 'b':
	    trig_thr = atof(optarg)/8192*14;            
            t_params[TRIG_SRC_PARAM] = 1;
	    t_params[TRIG_MODE_PARAM] = 1;
            t_params[TRIG_EDGE_PARAM] = 0;
	    t_params[TRIG_LEVEL_PARAM]= trig_thr;	
	...
	int idx;
        ...
        while(retries >= 0) {
		 ...
		 idx=(i-7+sig_len) % sig_len;   // FPGA delay correction
		  
                    printf(" %7d %7d\n",   (int)s[1][idx], (int)s[2][idx]);
                }
                break;
            }
